...
public class WorldCup {
	...
	public static void main(String[] args) {
		...
		// 8강
		for(int i=0; i<one.length; i+=2) {	// i = 0, 2, 4, 6
		...
		// 4강
		for(int i=0; i<two.length; i+=2) {	// i = 0, 2
		...
		// 결승
		for(int i=0; i<three.length; i+=2) {	// i = 0
		...
		for(int i=0; i<one.length; i+=4) {	// i = 0, 4
		...
		for(int i=0; i<one.length/2; i+=4) {	// i = 0, 4
		...
		System.out.println();
		System.out.println("최종선택은 "+one[0]+"입니다.");*/
		
		// 방법3 : 배열 및 반복문 한번만 사용하기
		String one[] = {"1", "2", "3", "4", "5", "6", "7", "8" };
		Scanner scan = new Scanner (System.in);
		String final_choice = null;
		...
		for(int i=0; i<3; i++) {
			...
			for(int j=0, x=0; j<length; j+=2, x++) {
				...
				if(choice == (j+1) || choice == (j+2)) {
					...
					System.out.println("잘못입력하셨군요..."+(j+1)+" 또는 "+(j+2)+"만 입력해주세요...");
					System.out.println();
					j -=2;
					x-=1;
					continue;
